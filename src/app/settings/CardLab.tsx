...
/**
 * The card lab: twenty ways the four library doors could look, side by side.
 *
 ...
 *
 * ONE markup, twenty stylesheets. Every variant renders the identical DOM and
 ...
 *
 * The second ten were added after the first ten were seen together, and they
 * are deliberately further apart: the first round circled one idea (an object
 * on a coloured field) too closely to settle anything.
 */
...
/** The twenty directions, in the order they are shown. */
const STYLES: { id: string; name: string; note: string }[] = [
  ...
  },
  {
    id: "marquee",
    name: "Marquee",
    note: "A theatre sign: warm bulbs across the top, wide caps, everything lit from the front. The friendliest of the twenty and the least like the rest of the app.",
  },
  {
    id: "blueprint",
    name: "Blueprint",
      ...
      <header className="cardLab__bar">
        <div className="cardLab__title">
          ...
          <Text tone="muted" size="xs">
            Twenty directions for the four library doors, on your own library
          </Text>
        </div>
        ...
        {shown.map((style, i) => (
          <section key={style.id} className="cardLab__style">
            <div className="cardLab__head">
              <span className="cardLab__no">
                {String(STYLES.indexOf(style) + 1).padStart(2, "0")}
              </span>
              <div>
                ...
                >
                  ...
                  {(style.id === "mosaic" || style.id === "contact") && (
                    <span className="labCard__mosaic" aria-hidden>
                      {(covers.length ? covers : Array(9).fill(null))
                        .slice(0, 9)
                        .map((src, n) =>
                          src ? (
