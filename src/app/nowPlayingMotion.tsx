...
export function NowPlayingMotionProvider({ children }: { children: ReactNode }) {
  const [source, setSource] = useState<MotionSource>({ meter: null, audible: false });
  ...
  const publish = useCallback((next: MotionSource) => {
    setSource((prev) =>
      prev.meter === next.meter && prev.audible === next.audible ? prev : next,
    );
  }, []);

  const value = useMemo<NowPlayingMotionValue>(
    () => ({ meter: source.meter, audible: source.audible, publish }),
    [source, publish],
  );
