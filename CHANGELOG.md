...
- Under the floor: a thousand automated checks now run over the app before a
  release goes out, so a fix to one screen stops quietly breaking another.
...
- The song menu's "More" opens as a sheet on the phone, with every row in
  view - it used to hang off the edge of the screen.
...
- The smart shuffle mark: a dark sparkle on the accent, with clear space
  cut out around it rather than an outline.

## 0.5.110

- The smart shuffle mark wears the Play button's colours: a dark sparkle on
  the accent.

## 0.5.109

- The smart shuffle mark is a small circle at the corner of the shuffle icon
  now, with room around it, and the icon itself matches ordinary shuffle.
...
- The DJ voice picker now lists every voice on your server's account, your own
  recordings at the top - so a voice you cloned is one you can choose.
...
- Listen along with a friend. When someone on your friends list is playing on
  the same server as you, their row shows “Listen along” — tap it and they get
  asked; their yes starts a shared session in sync, their player keeping time.
  You answer an incoming ask on a card under your profile.
...
- The icons in the navigation bar are a size larger, plugin ones included.
...
- Device storage: the "Still downloading" figure that never cleared is renamed
  to "Other files" and is what it always was - bytes on disk the cache no
  longer tracks (a download from a server you left, a vault from before a
  library moved), not a download in progress. Clearing downloads reclaims it.
...
- Profiles are global now. While sharing is on, your listening profile - the numbers, top artists, songs and albums for every range, and your liked songs by name - lives on your AttackFM account, not on a server. Friends see it from any server they listen on; what they own of it plays, the rest is read. Turning sharing off shuts the door at once.
...
- The invite card opens in the same pull-down glass sheet as search, is wider
  with a bigger code and QR, and saves at much higher resolution.
...
- The invite-a-friend button is now a plain share glyph in the header, just
  right of the notification bell, instead of a labelled button on the Library.
...
- The soft blur behind the lyrics now runs the full width of the screen, instead of stopping short with a sharp strip down each side.
...
- The "what's new" card no longer breaks a single note across several rows,
  each with its own icon - a wrapped line was being read as a fresh bullet.
  One note is now one row, on the update card and in the About timeline alike.
...
- Fixed liked songs that stayed stuck on "still downloading" forever: when the
  file landed crediting more artists than the heart was promised for
  ("CZARFACE, Frankie Pulitzer" against "Czarface"), the song was never
  recognised as arrived. It is now, and the heart settles.
- Liked songs opens in the order you liked them, newest first.
- The arriving songs read as rows of the list instead of a card on top of it,
  each saying whether it is queued or failed - and a failed one offers Try
  again.
...
- The AttackFM folder now takes over the moment you allow it - granting
  storage access mid-session used to leave downloads in the app's private
  space until the next full restart. Anything already cached moves in too.
- The storage pane notices the grant as soon as you come back from settings,
  instead of sitting on the Allow button.
...
- The Music Date numbers agree now: the library card and the date page both count your downloaded dates AND the preview pool
...
- Liking a song now sends a giant heart rippling out from the heart button - the outline swells across the screen in the album's own colour and fades as it goes. Unliking stays quiet, as it should
...
- The DJ's voice is properly audible now - every clip is lifted to broadcast loudness when it is made
- When the DJ finishes talking, the waves ride off the edge of the screen instead of blinking out
...
- Music Date deals in songs from today's charts, one card in three
- Artist radio stations stopped admitting strangers: a track must actually sound like the artist to join their station
...
- The primary colour glides between songs now, everywhere at once, instead of snapping
...
- The album colour is now a setting: Appearance > Album colour while playing, on by default
...
- The Taste page's share bar no longer runs off the card's rounded corners, and switching its pages starts each one at the top
- The trending research keeps a bounded pool, so an established library's years of candidates no longer crowd the new lanes out
...
- New music for you: the curator's finds, grouped into named sets on your Library page — songs you do not own yet, each with why it is there and a button to go and get it
- Tap a song's cover to hear the catalogue's thirty-second clip without stopping what is playing
...
- A pass in flight shows what it is doing and how long it has been going, and keeps updating while you watch
- The pane says less: the explanations of how it all works have moved out of the way of the controls

## 0.4.92

- Three Local AI functions that were quietly reporting "never run" are attributed to their work again: their names had drifted from the code
